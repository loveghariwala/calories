  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 font-sans">
      {CATEGORIES.map((cat) => {
        ...
        return (
          <InteractiveTilt key={cat.slug} maxTilt={8} scale={1.03} className="h-full">
            <Link href={`/category/${cat.slug}`} className="block group h-full">
              <div className="editorial-card rounded-3xl p-5 h-full flex flex-col justify-between hover:border-[#C4552D] transition-all duration-300 cursor-pointer shadow-xs">
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-3xl p-2.5 rounded-2xl bg-[#FAF8F5] border border-[#EAE3D9] shadow-2xs group-hover:scale-110 transition-transform">
                      {cat.emoji}
                    </span>
